...
官方标准解读与标准项目技术内容共同给出了一条可落地的完整链路：GB/T 45541-2025 的官方解读明确包含基本检查、基础测试、性能测试和测试报告要求[12]；GB/T 46104-2025 的官方项目页面进一步列出测试计划、数据采集计划、试验前检查、仪器精度、测量方法、稳态/动态/启停测试，以及包含测试目的、仪器设备、执行者姓名与资质、计算方法、图表和结论的试验报告。[8][13] 这直接决定了下一版产品应增加“合规准备度”门控：缺少适用标准、方法版本、仪器/校准、操作人员、环境与测试计划时，只能输出“分析结果”，不能输出“标准符合性结论”。
...
性能测试也不是一个通用方法。GB/T 45541-2025是现行的 PEM 电解槽性能测试方法；官方登记页确认其标准身份、现行状态和方法类别，官方解读页可直接核对其基本检查、基础测试、性能测试和测试报告四段流程，以及额定产氢压力和单槽额定产氢量的公开适用范围。[4][12] 具体仪器精度、试验条件、应急措施和计算条款不从公开摘要推定，仍需标准全文或企业批准方法资料。GB/T 46104-2025进一步面向电解水制氢系统的功率波动适应性测试，官方项目技术内容页公开列出测试条件、试验准备、电压/电流/电功率等电输入测量、稳态/动态/冷/热启动/停机和报告字段。[8][13] 这说明当前 H₂ TestLens 的温度/压力/泄漏/电压字段只是分析底座，不能把电压×电流派生量冒充原始电功率通道，也不足以单独宣称 PEM 电解槽性能符合标准。
...
分析阶段应固定计算方法、稳态窗口、过滤/缺失值规则、单位换算、派生量公式和不确定度/误差说明。TestLens 在存在原始 `power_w` 通道时优先使用该通道积分电能，并用电压×电流做独立交叉核算；没有原始功率通道的普通演示数据才使用明确标记的派生功率。当前能给出数据完整率、功率、电压稳定性、峰值温度/压力/泄漏、压力漂移、时间引用和批次差值；这些是可解释分析，不是自动替代企业标准计算的合规判定。
...
| 测试计划/采集计划 | UI/profile 已支持 testPlanRef、acquisitionPlan、preCheckRecord；46104 profile 还要求原始 power_w 通道及单位 | 用企业批准模板约束采样/同步/工况字段和原始电功率一致性规则 |
| 仪器/校准/执行者 | UI/profile 已支持 instrumentIds、calibrationRefs、operator、environment | compliance readiness 缺失即禁止标准符合性结论 |
| 计算方法/不确定度 | UI/profile 已支持 formulaRefs、uncertaintyPolicy | 仍需企业提供具体公式和不确定度计算规则 |
| 原始数据引用 | UI/profile 已支持 rawDataRef | 生产系统接入后保存企业对象路径或哈希，不上传原始行 |
...
[12] 国家市场监督管理总局、国家标准化管理委员会 (2025). “GB/T 45541-2025《PEM电解槽性能测试方法》国家标准解读.” https://std.samr.gov.cn/search/videoDetailed?id=33D629C63896064EE06397BE0A0A07F1 (Retrieved: 2026-08-21)
[13] 国家市场监督管理总局、国家标准化管理委员会 (2024). “电解水制氢系统功率波动适应性评价方法（国家标准项目技术内容）.” https://std.samr.gov.cn/gb/search/gbDetailed?id=FADA553B002CB31CE05397BE0A0A25A5 (Retrieved: 2026-08-21)
