...
性能测试也不是一个通用方法。GB/T 45541-2025是现行的 PEM 电解槽性能测试方法，官方平台列出仪器、测试条件、操作、应急、功率、产氢量、纯度、单位制氢电耗和耐压测试计算等内容；官方解读页进一步说明其覆盖基本检查、基础测试、性能测试和测试报告。[4][12] GB/T 46104-2025进一步面向电解水制氢系统的功率波动适应性测试，包含稳态、动态、冷/热启动、停机等测试类型和试验报告要求。[8][13] 这说明当前 H₂ TestLens 的温度/压力/泄漏/电压字段只是分析底座，不足以单独宣称 PEM 电解槽性能符合标准。
...
| 测试计划/采集计划 | UI/profile 已支持 testPlanRef、acquisitionPlan、preCheckRecord | 用企业批准模板约束采样/同步/工况字段 |
