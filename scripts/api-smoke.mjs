...
try {
  ...
  if (profileResult.rows !== undefined || profileResult.config?.profileName !== '企业电解槽 · 示例配置' || profileResult.schema?.mapping?.pressure_bar !== '压力(kPa)' || profileResult.compliance?.status !== 'DEMO_ONLY' || profileResult.compliance?.requiredMeasurements?.includes('hydrogen_purity_pct') !== true || !Array.isArray(profileResult.compliance?.requiredPhases) || profileResult.compliance?.acceptanceCriteria === undefined || profileResult.config?.uncertaintyModelRequired !== false || profileResult.config?.uncertaintyModel !== null) throw new Error('profile package was not applied by API');
