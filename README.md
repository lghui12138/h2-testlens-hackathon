...
- 本地 CSV 导入和演示样本加载
- 企业资料结构适配：车辆 `FC_*` CSV、中文电堆 CSV、GB18030 制表符 TXT；支持多文件 CSV/TXT 批次导入；原始企业数据不进入仓库
...
企业资料的本机盘点、真实字段适配和未完成项见 [`docs/ENTERPRISE_DATA_INTEGRATION.md`](docs/ENTERPRISE_DATA_INTEGRATION.md)。当前页面已经支持参数工作簿读取和 Excel 报告导出；仍未接入企业批准阈值、自动每日增量调度、飞书告警、服务端审计和 Excel 内嵌原生图表。
