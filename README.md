...
- 2026-08-24 继续轮：修复单位制氢电耗 1000 倍偏差，`Wh/NL` 与 `kWh/Nm³` 数值等价；阶段覆盖率按各会话时长求和，避免时间重置产生 200% 假覆盖；新增 AI 证据清理、API 路径边界、版本化 release receipt 和首屏发布状态。当前门：`npm test` **161/161**、`npm run check:submission` **127/127**、`npm run typecheck`、Vinext 生产构建 **5/5 阶段**、package smoke 全通过；仍不改变 `DEMO_ONLY` 边界。
