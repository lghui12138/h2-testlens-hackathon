...
当前交付版本：**v3.5.37**。本轮继续优化 GitHub Pages / App 的真实发布运行时：静态页和 Vinext 页都提供结果播报、图表文字摘要与 44px 移动导航目标；实际被 Vinext 加载的 `public/src` 四个浏览器模块已与 `src/` 强制 parity，避免 GitHub 网站只更新源码而不更新运行时。效率范围/全量覆盖、原始功率、连续稳态窗口和跨会话覆盖门继续保持 fail-closed。仍不把标准流程映射写成完整符合性声明。
...
- 2026-08-24 继续轮：修复单位制氢电耗 1000 倍偏差，`Wh/NL` 与 `kWh/Nm³` 数值等价；阶段覆盖率按各会话时长求和，避免时间重置产生 200% 假覆盖；批准分析拒绝部分原始功率、跨会话 steady 和不足两条稳态样本；效率实测值增加 0–100% 与批准覆盖率门；本轮又补齐 GitHub App 实际加载的 `public/src` 四个运行时模块 parity、静态/App 结果播报、三类画布文字摘要和 44px 移动导航触控目标，并把 parity 加入提交门。当前门：`npm test` **165/165**、`npm run check:submission` **128/128**、`npm run typecheck`、Vinext 生产构建 **5/5 阶段**、package smoke 全通过；仍不改变 `DEMO_ONLY` 边界。
