...
当前交付版本：**v3.5.36**。本轮修复车辆跨午夜/跨会话时间口径、电堆正式点质量门和派生功率验收漏洞，并补齐网页与 Next/Vinext App 的渲染 parity；仍不把标准流程映射写成完整符合性声明。

交付形态：浏览器网页静态入口 `src/index.html` 与 Next/Vinext App 入口 `app/page.tsx` 均提供同一套分析功能；当前不包含原生 iOS/Android 安装包或原生设备能力保证。

v3.5.36 证据：`.research/ignite_t02_standards_20260821/t02_coverage_audit_v3.5.36.json`、`.research/ignite_t02_standards_20260821/t02_reference_audit_v3.5.36.json`、`.research/ignite_t02_standards_20260821/t02_integration_report_v3.5.36.md`。
...
- v3.5.36 当前实测：T02 三个示例 profile 固定为 `descriptive_only`、未审批和 `thresholds: null`；静态网页、Next/Vinext App、API、`batch-watch` 和 T02 全资料覆盖审计均保持该模式，170 个车辆源文件进入动态设定变化描述分析，当前汇总 15,281 个事件，其中 3,727 个进入响应带并满足保持窗口，11,554 个未稳定，0 个超过配置上限的采样缺口；车辆时间计算按会话本地轴，正式功率验收拒绝派生功率；该分析按源文件/会话隔离，不执行阈值、验收、安全、符合性或放行判定。标准符合性、企业批准 profile、Excel/WPS 视觉验收和真实平行验证仍未完成。
