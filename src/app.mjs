...
function renderEnterprisePanel(result) {
  ...
  $('#enterprise-summary').innerHTML = `<div class="enterprise-facts"><span><b>${dataset.cellChannelCount}</b> 个导出单片通道</span><span><b>${dataset.configuredCellCount || '—'}</b> 片数参数</span><span><b>${fmt(dataset.metrics.averageCurrentA, 2)}</b> A 平均电流</span><span><b>${fmt(dataset.metrics.cellSpreadMaxV, 3)}</b> V 最大极差</span><span><b>${dataset.platforms?.length || 0}</b> 个${dataset.parameterConfig?.ok ? '电流平台' : '描述性候选区间'}</span><span><b>${dataset.performancePoints?.length || 0}</b> 个正式稳定区间</span><span><b>${fmt(dataset.metrics.hydrogenStoich?.mean, 3)}</b> 氢气计量比</span><span><b>${fmt(dataset.metrics.airStoich?.mean, 3)}</b> 空气计量比</span><span><b>${dataset.coverage?.totalSignals || dataset.signalCatalog?.length || 0}</b> 列字段目录</span></div><div class="enterprise-note">核心/交叉核对 ${dataset.coverage?.analysisSignals ?? '—'} 列 · 仅目录 ${dataset.coverage?.catalogOnlySignals?.length ?? '—'} 列 · 设定/实测核对 ${dataset.settingCrossChecks?.length || 0} 组 · 阀/泵反馈 ${dataset.feedbackSignals?.length || 0} 列${dataset.parameterConfig?.ok ? '' : ' · 未提供目标参数，未生成正式极化点'}</div>`;
  ...
  $('#enterprise-summary').innerHTML += `<h3>工程量摘要（统计/核对，不是标准结论）</h3><table><thead><tr><th>工程量</th><th>均值</th><th>单位</th><th>有效记录</th></tr></thead><tbody>${engineeringRows}</tbody></table><p class="enterprise-note">入口露点只接受原始字段明确标注为露点的记录；增湿罐水温单独列示，不能替代露点。循环水电导率无企业批准限值时只统计、不判定。</p>`;
