...
export function buildEnterpriseWorkbook(result, fileName = 'test-run.csv', options = {}) {
  ...
  const datasetLogRows = dataset.kind === 'vehicle'
    ...
    : dataset.kind === 'stack'
      ? [['电堆会话边界', '已记录', `${dataset.sessionCount || 1} 个会话；平台与稳定区间不跨文件拼接`], ['电堆稳定区间选择', '已记录', `策略 ${dataset.selectionPolicy || '未声明'}；人工改选 ${selectionRows.filter((row) => row[1] === '人工改选').length} 个平台`], ['电堆功率来源', '已记录', `${dataset.metrics?.powerSource || '—'}；${dataset.metrics?.powerCrossCheck?.evidence || '—'}`], ['电堆逐片时序统计', dataset.cellTimeSeriesStats?.length ? '已记录' : '未形成', `${dataset.cellTimeSeriesStats?.length || 0} 个单片通道；只作描述性统计`], ['排除区间证据', dataset.excludedIntervals?.length ? '已记录' : '无', `${dataset.excludedIntervals?.length || 0} 个区间；保留原因和时间边界`], ...(dataset.workbookStabilityCrossCheck ? [['XLSX稳定性核对', dataset.workbookStabilityCrossCheck.status, `${dataset.workbookStabilityCrossCheck.matchedCount || 0}/${dataset.workbookStabilityCrossCheck.reportPointCount || 0} 个报告点；${dataset.workbookStabilityCrossCheck.boundary}`]] : [])]
      : [];
