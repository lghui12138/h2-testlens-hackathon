...
export function buildEnterpriseWorkbook(result, fileName = 'test-run.csv', options = {}) {
  ...
  addSheet(book, '处理日志', [['步骤', '状态', '证据/来源'], ...(result.workflow?.steps || []).map((step) => [step.label, step.status, step.evidence]), ['参数工作簿', parameterConfig?.ok ? '已校验' : '未提供/未通过', parameterConfig ? `${parameterConfig.parameterSheet || ''}；${parameterConfig.targetSheet || ''}` : '需要导入企业参数工作簿'], ['批次文件 manifest', manifest.length ? '已记录' : '未提供', `文件 ${manifest.length}；新增 ${diff.added.length}；变更 ${diff.changed.length}；未变 ${diff.unchanged.length}；移除 ${diff.removed.length}`], ...manifest.map((entry) => [`文件：${entry.name}`, '批次证据', `记录 ${entry.rowCount ?? '—'}；大小 ${entry.size ?? '—'}；SHA-256 ${entry.hash || '—'}`]), ['报告边界', '必须保留', '不替代企业标准负责人、测试人员或实验室签核']]);
