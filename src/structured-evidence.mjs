...
export function efficiencyReadiness(config = {}, rows = [], schema = {}) {
  ...
  const invalidMeasuredValues = measuredValues.filter((value) => value < 0 || value > 100);
  const minimumSamples = Number.isFinite(Number(requirement.minimumSamples)) ? Math.max(2, Number(requirement.minimumSamples)) : 2;
  const minimumCoveragePct = Number.isFinite(Number(requirement.minimumCoveragePct)) ? Number(requirement.minimumCoveragePct) : config.approvalStatus === 'approved' ? 100 : null;
  const measuredCoveragePct = rows.length ? (measuredValues.length / rows.length) * 100 : 0;
  const measuredProblems = [];
  if (invalidMeasuredValues.length) measuredProblems.push(`efficiency_pct 超出 0–100% 范围：${invalidMeasuredValues.length} 条`);
  if (measuredValues.length < minimumSamples) measuredProblems.push(`有效效率记录不足 ${minimumSamples} 条：${measuredValues.length} 条`);
  if (minimumCoveragePct !== null && measuredCoveragePct < minimumCoveragePct) measuredProblems.push(`效率记录覆盖率 ${measuredCoveragePct.toFixed(1)}% < ${minimumCoveragePct}%`);
  const measured = measuredProblems.length === 0 && measuredValues.length >= minimumSamples ? measuredValues.reduce((sum, value) => sum + value, 0) / measuredValues.length : null;
  ...
  const recordValueOutOfRange = numericRecordValue !== null && (numericRecordValue < 0 || numericRecordValue > 100);
