...
export function buildT02IntegrationReport({ coverage, reference, coveragePath = 't02_coverage_audit.json', referencePath = 't02_reference_audit.json' }) {
  ...
  const fileRows = records.map((record, index) => {
    ...
    return `| ${index + 1} | ${cell(record.package)} | ${cell(record.path)} | ${cell(statusLabel(record.status))} | ${cell(record.parser)} | ${count(record.rowCount)} | ${cell(dispositionLabel(record.usageLedger?.disposition))} | ${boolLabel(record.usageLedger?.parserExecuted)} | ${count(record.usageLedger?.rowsEnteredAdapter)} | ${cell(referenceAuditStatus)} | ${count(referenceClaimCount)} | ${boolLabel(record.usageLedger?.formalConformityClaim)} |`;
  }).join('\n');
  ...
  const lines = [
    ...
    '## 全文件使用台账',
    '',
    '| # | 资料包 | 文件 | 状态 | 原始数据解析器 | 行/功率点 | 用途层级 | 执行原始解析器 | 进入适配器 | 参考内容审计 | 参考声明数 | 正式符合性声明 |',
    '|---:|---|---|---|---|---:|---|---|---:|---|---:|---|',
    fileRows || '| — | — | — | — | — | — | — | — | — | — | — | — |',
    '',
    '## 未进入时序适配器的文件',
