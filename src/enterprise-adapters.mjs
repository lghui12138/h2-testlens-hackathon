...
function buildVehicle(rows, config) {
  ...
  const rowsForAnalysis = analysisRows.length || !requestedWindow ? (analysisRows.length ? analysisRows : normalized) : normalized;
  ...
  const vehicleTrend = (points, field) => vehicleTrendMetric(points, field, selectedTrendAxis, trendModel);
  ...
  const metrics = {
    ...
    durationS: sessionTime.sessions.length > 1 ? null : windowEndS - windowStartS,
    completenessPct: quality.completenessPct,
    peakPowerW: Math.max(...rowsForAnalysis.map((row) => row.net_power_kw).filter((value) => value !== null), 0) * 1000,
