...
function profileGateIssues(complianceResult) {
  const issues = [];
  if (complianceResult.approvalStatus === 'example_unapproved') issues.push({ severity: 'critical', code: 'STANDARD_COMPLIANCE_BOUNDARY', title: '企业未审批 profile 不构成标准符合性判定', evidence: '当前使用未经企业审批的 profile，不构成标准符合性判定或放行依据', recommendation: '需先完成企业审批、修订控制及完整方法实施证据，再进入标准符合性复核。' });
  ...
  if (complianceResult.dataQuality?.ready === false) issues.push({ severity: 'critical', code: 'DATA_QUALITY_GATE_FAILED', title: 'profile 要求的数据质量门控未通过', evidence: complianceResult.dataQuality.evidence, recommendation: '修复时间轴、采样间隔或阶段连续性后重新计算。' });
...
function compliance(config, datasetLabel, metrics, quality, rows = [], schema = {}) {
  ...
  const standardBreakdown = (config.standardRefs || []).map((reference) => {
    ...
    return {
      ...
      boundary: clauseRefs.length
        ? `该标准引用当前仅映射公开范围与公开流程，不作完整符合性判定；所列条款（${clauseRefs.join('、')}）需企业批准的 profile 与完整方法实施证据支撑。`
  ...
  const evidenceDetails = [
    ...
    { id: 'testSystem', label: '测试装置组成证据', ready: testSystem.ready, evidence: testSystem.evidence },
...
function workflow(dataset, quality, issues, targetConfigured = true, complianceResult = null) {
  ...
  const steps = [
    ...
    { id: 'test-system', label: '测试装置组成证据', status: complianceResult?.testSystem?.ready ? 'ready' : 'needs_input', evidence: complianceResult?.testSystem?.evidence || '尚未核对' },
...
function buildDurability(rows, config) {
  ...
  if (!Number.isFinite(maxDeviationMv) && !Number.isFinite(minAverageCellVoltageMv)) issues.push({ severity: 'info', code: 'DURABILITY_RULES_NOT_CONFIGURED', title: '耐久预警阈值未配置', evidence: '当前只保留企业原始测试结果和统计，不自动新增异常判定', recommendation: '由企业负责人填写离均差/平均单体电压预警规则后重新计算。' });
...
function buildVehicle(rows, config) {
  ...
  if (unresolvedUnitFields.size) issues.push({ severity: 'warn', code: 'VEHICLE_UNIT_UNRESOLVED', title: '车辆单体电压单位未确认', evidence: `${[...unresolvedUnitFields].join('、')} 的原始值超出 V 口径可直接解释的范围，已不进入单体电压均值/趋势；原始值保留在 raw 字段`, recommendation: '在企业 profile 中提供 vehicleSignalUnits.sourceUnit（V 或 mV）及证据后重新计算；不按数量级自动换算。' });
  ...
  if (!targetCurrents.length) issues.push({ severity: 'warn', code: 'VEHICLE_TARGETS_MISSING', title: '未提供性能统计目标电流', evidence: `已发现 ${inferredSegments.length} 个连续电流候选区间，但这些区间仅按实际数据推断，未生成正式目标电流性能点`, recommendation: '导入企业批准的目标电流、允许波动范围和最短持续时间后重新计算；当前候选只用于描述性复核。' });
...
function buildStack(rows, config) {
  ...
  const issues = [];
  if (unsupportedUnitFields.length) issues.push({ severity: 'critical', code: 'UNIT_UNSUPPORTED', title: '电堆表头单位无法安全换算', evidence: unsupportedUnitFields.join('；'), recommendation: '补充明确的 canonical 单位或企业单位字典；不按数值大小猜测单位。' });
  if (hasUsableTargetConditions && !quality.usable) issues.push({ severity: 'critical', code: 'STACK_DATA_QUALITY_BLOCKS_POINTS', title: '时间轴/关键字段质量未通过，未生成正式稳定点', evidence: `重复时间戳 ${quality.duplicateTimestampCount} 条，逆序 ${quality.nonMonotonicCount} 次，非正间隔 ${quality.nonPositiveIntervalCount} 个；正式目标工况点已阻断`, recommendation: '修复时间轴和关键字段后重新计算；不使用配置采样周期覆盖无效原始时间戳。' });
  ...
  if (signalDiagnostics.reviewSignalCount) issues.push({ severity: 'warn', code: 'SIGNAL_VALUE_SEMANTICS_REVIEW', title: '原始信号值分布需要语义复核', evidence: `${signalDiagnostics.reviewSignalCount} 个核心/交叉核对字段存在负值、零值集中、常量值或数值解析混合；示例：${signalDiagnostics.reviewSignals.slice(0, 4).map((item) => `${item.source}（${item.reasons.join('、')}）`).join('；')}`, recommendation: '先确认字段单位、设备状态和企业无效码表；不自动删除负值/零值，也不把该提示直接转为不合格。' });
