...
function enterpriseReportMarkdown(result, fileName, options = {}) {
  ...
  if (result.datasetType === 'vehicle') {
    ...
    const trends = Object.entries(dataset.performanceTrend || {}).map(([field, trend]) => `- ${field}：${trend.pointCount} 点；斜率/日 ${trend.slopePerDay ?? '—'}`).join('\n');
