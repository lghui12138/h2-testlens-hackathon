...
export const OPTIONAL_FIELDS = Object.freeze(['phase']);
...
function complianceReadiness(config) {
  ...
  const status = config.approvalStatus !== 'approved' ? 'DEMO_ONLY' : missingProfileFields.length || missingMetadata.length ? 'NOT_READY' : 'READY_FOR_HUMAN_REVIEW';
...
export function analyzeRows(inputRows, suppliedConfig = {}) {
  ...
  const compliance = complianceReadiness(config);
  ...
  if (schema.missingOptionalHeaders.length) issues.push(issue('info', 'PHASE_OPTIONAL', '未提供工况字段', '将使用活动窗口回退估计稳态段', '如企业日志有工况/阶段字段，可在字段映射中补充。'));
