...
function applyEvaluationMode(result) {
  ...
  const issues = (result.issues || []).map((item) => DESCRIPTIVE_DECISION_CODES.has(item.code)
    ...
    : item);
  if (!issues.length) issues.push({ severity: 'info', code: 'DESCRIPTIVE_ONLY', category: 'evaluation_boundary', title: '仅完成描述性统计', evidence: '已输出字段映射、数据质量、统计指标、趋势和证据；未执行阈值或验收判定。', recommendation: '如需风险筛查或正式验收，需导入企业批准的 risk_screening/acceptance profile。' });
  return {
    ...result,
    ...
    workflow: {
      ...
      nextAction: '当前仅供工程师描述性复核；如需风险筛查或正式验收，需导入企业批准 profile。',
...
function resolveSchema(inputRows, explicitMapping = {}) {
  ...
  for (const field of [...REQUIRED_FIELDS, ...ADDITIONAL_STANDARD_FIELDS]) {
    ...
    if (candidates.length > 1) mappingWarnings.push(`${field} 存在多个候选表头（${candidates.join('、')}），已选择“${sourceHeader}”；需确认企业字段映射。`);
...
function workflowReadiness(config, quality, schema, compliance, verdict, uncertainty) {
  ...
  const steps = [
    ...
    { id: 'test-system', label: '测试装置组成证据', status: testSystemReady ? 'ready' : 'needs_input', evidence: compliance.testSystem?.evidence || '尚未形成测试装置组成证据' },
...
export function analyzeRows(inputRows, suppliedConfig = {}) {
  ...
  if (enterpriseResult) {
    ...
    if (workbookFormulaReview.audit?.formulaCellCount > 0) {
      enterpriseResult.issues = [{
        ...
        recommendation: workbookFormulaReview.required && !workbookFormulaReview.ready
          ? '补充 reviewerId、reviewedAt、evidenceRef、sourceHash 和复核决定；未复核缓存值不用于正式验收。'
    ...
    if (supported.length && !supported.includes(enterpriseResult.datasetType)) {
      ...
      enterpriseResult.narrative = '当前数据集与所选企业 profile 的适用范围不一致，后续结论已阻断。';
  ...
  if (unsupportedFields.length) issues.push(issue('critical', 'UNIT_UNSUPPORTED', '输入单位无法安全换算到分析标准单位', unsupportedFields.map(([field, transform]) => `${field}：${transform.label}`).join('；'), '补充标准状态/温度/压力基准，或将原始通道导出为 canonical 单位后再分析；不会猜测实际体积流量与标准体积流量的等价关系。'));
  ...
  if (convertedFields.length) issues.push(issue('info', 'UNIT_CONVERTED', '已完成输入单位换算', convertedFields.map(([field, transform]) => `${field} ${transform.label}`).join('；'), '正式归档前确认原始表头和单位定义。'));
  ...
  if (quality.missingCells > 0 && quality.missingHeaders.length === 0) {
    ...
    issues.push(issue('warn', 'DATA_GAP', '测试数据存在缺口', `关键数值字段缺失 ${quality.missingCells}/${quality.totalCells} 个单元格（完整率 ${quality.completenessPct.toFixed(1)}%）；${missingFields}`, '补齐缺失字段或提供企业批准的缺失值规则后再做正式判定；不会用插值或默认值替代原始记录。'));
  }
  ...
  if (config.approvalStatus === 'approved' && testStages.required && !testStages.ready) issues.push(issue('critical', 'TEST_STAGE_EVIDENCE_MISSING', '标准测试阶段证据不完整', testStages.evidence, '为 profile 要求的每个测试阶段填写完成状态和证据引用；自由文本计划不视为阶段完成。'));
  if (config.approvalStatus === 'approved' && testSystem.required && !testSystem.ready) issues.push(issue('critical', 'TEST_SYSTEM_EVIDENCE_MISSING', '测试装置组成证据不完整', testSystem.evidence, '为 profile 要求的装置组成项目填写通过状态和证据引用；适用性由企业 profile 确认。'));
  if (config.approvalStatus === 'approved' && testConditions.required && !testConditions.ready) issues.push(issue('critical', 'TEST_CONDITION_EVIDENCE_MISSING', '测试条件或异常处置证据不完整', testConditions.evidence, '补充测试场所、环境条件、装置组成确认和异常处置引用；具体条件值由企业 profile/方法提供。'));
  if (config.approvalStatus === 'approved' && environmentConditions.required && !environmentConditions.ready) issues.push(issue('critical', 'ENVIRONMENT_CONDITION_EVIDENCE_MISSING', '环境条件证据不完整', environmentConditions.evidence, '补充 profile 要求的环境温度、湿度、气压等记录和证据引用；不猜标准条件或限值。'));
  if (config.approvalStatus === 'approved' && measurementMethods.required && !measurementMethods.ready) issues.push(issue('critical', 'MEASUREMENT_METHOD_EVIDENCE_MISSING', '测量方法证据不完整', measurementMethods.evidence, '为电输入、气体输出、环境条件和高频采集等 profile 要求提供方法引用、证据和测量字段覆盖；不猜仪器精度。'));
  if (config.approvalStatus === 'approved' && efficiency.required && !efficiency.ready) issues.push(issue('critical', 'EFFICIENCY_EVIDENCE_MISSING', '效率结果或批准公式缺失', efficiency.evidence, '提供实测 efficiency_pct，或提供企业批准的效率结果、公式引用和结果证据；不会自行推导效率。'));
  if (config.approvalStatus === 'approved' && phaseResults.required && !phaseResults.ready) issues.push(issue('critical', 'PHASE_RESULT_EVIDENCE_MISSING', '方法阶段结果证据不完整', phaseResults.evidence, '为 profile 要求的阶段提供可计算结果字段、阶段状态和证据引用；通用 KPI 不自动宣称为标准结果。'));
  ...
  if (config.approvalStatus === 'approved' && acceptance.status === 'failed') issues.push(issue('critical', 'ACCEPTANCE_RULE_FAIL', '企业验收规则未通过', acceptance.evidence, '先处置异常并按测试方法决定复测、偏差放行或不通过；不替代授权人员决定。'));
  ...
  const narrative = verdict === 'FAIL'
    ? `本次测试判定为需复核：发现 ${criticalCount} 项高优先级风险。异常指标、阈值和后续步骤已绑定到同一证据链，工程师可先处理安全相关项，再决定是否重测。`
    : verdict === 'WARN'
      ? '本次测试未触发高优先级安全规则，但存在需要工程师复核的趋势或数据质量问题，后续步骤：在正式归档前补测。'
      : '本次测试关键指标未触发当前规则，后续步骤：结合企业标准和设备工况完成最终签核。';
  const result = {
...
const PUBLIC_CONFIG_REDACTED_KEYS = new Set(['apiKey', 'password', 'secret', 'token', 'rawDataRef', 'webhookUrl', 'oldValue', 'newValue']);

function redactPublicConfigValue(value, depth = 0) {
  if (depth > 8 || value === null || value === undefined || typeof value !== 'object') return value;
  if (Array.isArray(value)) return value.map((item) => redactPublicConfigValue(item, depth + 1));
  return Object.fromEntries(Object.entries(value).filter(([key]) => !PUBLIC_CONFIG_REDACTED_KEYS.has(key)).map(([key, item]) => [key, redactPublicConfigValue(item, depth + 1)]));
}
...
export function publicConfig(config = {}) {
  ...
  const editLog = editLogReadiness(config);
  return {
    ...redactPublicConfigValue(safeConfig),
