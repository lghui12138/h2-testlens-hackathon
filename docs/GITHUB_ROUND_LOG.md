...
- 本地门控：`npm test` 184/184、`npm run check:submission` 136/136、AI grounding 4/4、API smoke、两个 profile audit、typecheck、Vinext build 5/5、package smoke install/test/typecheck/build/start/HTTP 全部通过；包 SHA-256 由 `dist/h2-testlens-submission-v3.5.38.zip.sha256` 记录；新增精确哈希接受/错哈希阻断回归。
...
- 演示样本加载现在捕获资源 404/网络失败，显示错误原因和手动导入恢复建议；电堆 `CELL1(mV)` 等单片通道先换算到 V，避免 700 mV 被当成 700 V。
- GitHub Pages T02 卡片现在直接显示前 5 个高风险字段的文件数、负值数和零值数；这些是字段语义/无效码复核提示，不是异常或标准判定。
- 车辆正式目标电流性能点现在默认只接受 `FC_MainSts=4` 活动状态；状态 8 等非活动记录保留作描述性/绝缘统计，并输出排除证据。
...
- Run 84 之前只证明 Pages artifact 发布成功；当前 `prepare-github-pages` 已在 Actions 缺少 receipt 时自动执行 package smoke，自定义域仍不可达。
...
- XLSX 导出在未载入结果时现在与 Markdown/JSON 一样明确提示；带 `standardRefs + methodSource` 的正式配置缺少 runtime binding 时 fail-closed。
