...
- 已实现：生成 14 个工作表的 Excel 报告，保留有效参数、字段映射、质量检查、平台/稳定区间、异常和处理日志；目标/实际偏差含 Excel 公式并已通过零公式错误复算检查。
- 已实现：profile 可提供 `first_order_rss` 不确定度模型；系统仅在模型存在时传播峰值、产氢量、电能和单位能耗不确定度，未配置时明确显示未配置。
- 未实现：企业批准的具体验收限值、测量不确定度计算规则、Excel 原生嵌入图表、实验室/工单系统和正式电子签章。这些必须由企业资料包提供，不能由演示模板推断。
