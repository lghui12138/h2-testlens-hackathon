...
- Gate: `npm test` **183/183**, `npm run check:submission` **136/136**, `npm run eval:ai` **4/4**, API smoke, T02 audits, typecheck, Vinext build 5/5 and package smoke install/test/typecheck/build/start/HTTP pass. The v3.5.38 package receipt/sidecar remains the artifact source of truth and is kept outside the embedded self-reference path.
