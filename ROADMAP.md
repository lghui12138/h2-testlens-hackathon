...
> Current top-level status: **v3.5.38**. The historical phase rows below retain their original scope wording; current verification is recorded in the v3.5.38 continuation entries.
...
## v3.5.38 current continuation evidence
...
- Trusted approval package binding now hashes the complete author-controlled profile package, including `approvalStatus` and `approvalEvidence`, while excluding only runtime-generated `trustedApprovalBinding`; API, browser import and `batch-watch` all pass the controlled SHA-256 into the ledger gate, and approval-package edits fail closed on mismatch.
- T02 coverage UX now exports the top five field-value review signals as a UTF-8 CSV with aggregate counts and review reasons only; the download is available on both static Pages and Vinext App and remains explicitly non-conformity evidence.
- Vehicle cell-voltage normalization now separates raw and physical KPI values: non-positive normalized cell voltages are retained in raw fields, excluded from physical voltage means/trends/performance points, counted under `VEHICLE_NONPOSITIVE_CELL_VOLTAGE_FILTERED`, and covered by regression tests.
- Stack anode/cathode flow mappings now require explicit standard-state units, public quality recursively redacts session source filenames, `excel-workflow.mjs` is parity-gated, and all 18 runtime evidence rows reconcile `standard_id` into the research JSONL graph.
- Vehicle `FC_VARVoltage` now has an independent `V²/mV²` unit contract and conversion path; one-power voltage units are rejected for variance fields and mV² conversion is regression-tested.
- Versioned metricTrace now links each exposed KPI to canonical source fields, an opaque metric evidence ID, input-hash binding status, evidence class, and privacy-safe file-token/row-range locators; malformed or absent hashes remain explicitly unbound.
- Runtime standard profile imports now require the repository-backed standard-evidence-ledger.v1 artifact when standardRefs + methodSource are declared; methodSource evidenceIds must match source_id, locator and evidence_type. API, browser profile import and batch-watch use this path; T02 descriptive profiles remain not_configured.
- Runtime standard profile imports now bind every declared `standardRefs[]` item independently with `evidenceSourceId/evidenceIds`; one method-source binding can no longer stand in for several listed standards.
- Generic flow parsing now fails closed for explicit actual `L/min` and `m³/h` headers unless a standard-state conversion is available; `SLPM`, `NL/min` and `Nm³/h` remain explicit canonical/convertible forms. The public runtime carries the same rule.
- Stack anode/cathode gas flow mappings now apply the same unit gate; explicit actual `L/Min` cannot enter hydrogen/air stoichiometry as if it were `SLPM`.
- T02 integration report rows now expose each source file's SHA-256 and field-role counts, and the public T02 coverage card links directly to the full report; mixed browser batches surface parser-error counts alongside processed/reference/binary counts.
...
- The trace is carried through publicAnalysis/API, batch and compare responses, AI evidence, Markdown, XLSX sheet KPI证据定位, JSON download and the App’s keyboard-accessible collapsed evidence table. Enterprise public issue/report surfaces now redact raw evidence text while retaining status/count information.
...
- Gate: `npm test` **188/188**, `npm run check:submission` **139/139**, `npm run eval:ai` **4/4**, API smoke, T02 audits, typecheck, Vinext build 5/5 and package smoke install/test/build/start/HTTP pass. Exact trusted-ledger hash acceptance/mismatch blocking, standard-id reconciliation, unitless-flow blocking, mixed positive/non-positive voltage filtering, explicit V/mV units, public quality redaction, Excel parity and full-SHA Node24 Pages action pins are regression-tested. The v3.5.38 package receipt/sidecar remains the artifact source of truth and is kept outside the embedded self-reference path.
- Residual: Actions Run `32712931265` (page #101) successfully executed the package-smoke hook and deployed the v3.5.38 Pages artifact at `https://lghui12138.github.io/projects/h2-testlens.html`; custom-domain HTTPS/live readback remains external.
...
- Verification artifacts: v3.5.38 T02 coverage/reference/integration reports; new per-cell, excluded-interval and parameter-formula regressions; full submission gates remain required before packaging.

## 2026-08-24 current continuation evidence · unit energy, session coverage and release receipt
...
- Gate: `npm test` **165/165**, `npm run check:submission` **127/127**, typecheck, build 5/5, package smoke and static preparation pass; the adjacent versioned release receipt contains the final artifact SHA-256.
