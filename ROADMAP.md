...
- Gate: `npm test` **176/176**, `npm run check:submission` **132/132**, typecheck, Vinext build 5/5 and package smoke install/test/typecheck/build/start/HTTP pass. API smoke now exercises AI client-parameter isolation, metricTrace, runtime method-source binding and per-standard-reference evidence binding. The package receipt remains the source of truth for the final artifact SHA-256.
- Residual: GitHub Actions Run 84 successfully deployed `a0de3f3`, but the Pages workflow still does not run `package:submission`; the public receipt can bind the deployed commit/run without claiming a package artifact. Final live receipt readback remains pending while the custom domain is unreachable.
