...
- T02 integration report rows now expose each source file's SHA-256, and the public T02 coverage card links directly to the full report; mixed browser batches surface parser-error counts alongside processed/reference/binary counts.
...
- Residual: GitHub Pages workflow still does not run `package:submission` because the current OAuth token lacks workflow scope; the public receipt can now bind the deployed commit/run without claiming a package artifact. Final live receipt readback remains pending while the custom domain returns 502.
