...
- Gate: `npm test` **186/186**, `npm run check:submission` **139/139**, `npm run eval:ai` **4/4**, API smoke, T02 audits, typecheck, Vinext build 5/5 and package smoke install/test/build/start/HTTP pass. Exact trusted-ledger hash acceptance/mismatch blocking, standard-id reconciliation, unitless-flow blocking, public quality redaction, Excel parity and full-SHA Node24 Pages action pins are regression-tested. The v3.5.38 package receipt/sidecar remains the artifact source of truth and is kept outside the embedded self-reference path.
