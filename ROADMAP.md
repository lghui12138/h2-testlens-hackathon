...
- Gate: `npm test` **184/184**, `npm run check:submission` **138/138**, `npm run eval:ai` **4/4**, API smoke, T02 audits, typecheck, Vinext build 5/5 and package smoke install/test/build/start/HTTP pass. Exact trusted-ledger hash acceptance/mismatch blocking and full-SHA Node24 Pages action pins are regression-tested. The v3.5.38 package receipt/sidecar remains the artifact source of truth and is kept outside the embedded self-reference path.
