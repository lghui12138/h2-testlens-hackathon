...
test('static and Vinext pages expose the same multi-format T02 input contract', async () => {
  ...
  assert.match(app, /downloadCoverageReview/);
  assert.match(app, /csvCell/);
  assert.match(app, /#compare-status.*先载入并计算数据/);
  assert.match(app, /#report-status.*先载入并计算数据/);
...
test('T02 coverage summary is parity-checked for static page runtime loading', async () => {
  ...
  assert.equal(summary.auditVersion, '3.5.72');
...
test('T02 coverage card is visible on both public browser surfaces', async () => {
  ...
  for (const page of [staticPage, rendered]) {
    ...
    for (const id of ['coverage-audit-version', 'coverage-total-files', 'coverage-processed', 'coverage-reference', 'coverage-blocked', 'coverage-unuploaded', 'coverage-meter-fill', 'coverage-note', 'coverage-risk-fields', 'download-coverage-review', 'coverage-review-status']) assert.match(page, new RegExp(id));
