...
test('batch observation records a rewind boundary when restart boundary is declared', () => {
  ...
  const entries = [
    ...
    { name: 'b.csv', rows: [{ timestamp_s: '10', phase: 'steady', current_a: '10', voltage_v: '20', temperature_c: '25', pressure_bar: '1', flow_slpm: '3', leak_ppm: '1' }] }
  ];
...
test('batch summary redacts identifiers but preserves descriptive boundary text', () => {
  ...
  assert.equal(publicSummary.testRunIdPresent, false);
  assert.equal(publicSummary.sessionGroupPresent, false);
  assert.ok(publicSummary.boundary === undefined || typeof publicSummary.boundary === 'string');
});
