...
test('stack adapter detects duplicate timestamps and reports timestamp resolution issue', () => {
...
test('compliance retains formal blockers for unapproved profiles with empty standardRefs', () => {
  ...
  assert.ok(result.boundary.includes('ENTERPRISE_PROFILE_REQUIRED'));
...
test('analyzeRows accepts values exactly at default thresholds', () => {
  ...
  assert.ok(result.verdict === 'PASS' || result.verdict === 'WARN' || result.verdict === 'FAIL');
  assert.ok(result.issues.some((issue) => issue.code === 'TEMP_HIGH') || result.verdict === 'PASS');
});
...
test('analyzeRows derives zero power from zero current and nonzero voltage', () => {
  const result = analyzeRows(parseCSV([
    'timestamp_s,current_a,voltage_v,temperature_c,pressure_bar,flow_slpm,leak_ppm,hydrogen_purity_pct',
    '0,0,2,30,10,6,1,99',
    '60,0,2,30,10,6,1,99'
...
test('batch observation records a forward boundary when restart boundary is declared', () => {
  const group = validateBatchDeclaration({
    ...
    groups: [{
      ...
      samplingTolerancePct: 20,
      restartBoundaries: ['b.csv']
    }]
  }, [{ name: 'a.csv', status: 'processed' }, { name: 'b.csv', status: 'processed' }]).groups[0];

  const entries = [
    { name: 'a.csv', rows: [{ timestamp_s: '0', phase: 'steady', current_a: '10', voltage_v: '20', temperature_c: '25', pressure_bar: '1', flow_slpm: '3', leak_ppm: '1' }, { timestamp_s: '1', phase: 'steady', current_a: '10', voltage_v: '20', temperature_c: '25', pressure_bar: '1', flow_slpm: '3', leak_ppm: '1' }] },
    { name: 'b.csv', rows: [{ timestamp_s: '2', phase: 'steady', current_a: '10', voltage_v: '20', temperature_c: '25', pressure_bar: '1', flow_slpm: '3', leak_ppm: '1' }, { timestamp_s: '3', phase: 'steady', current_a: '10', voltage_v: '20', temperature_c: '25', pressure_bar: '1', flow_slpm: '3', leak_ppm: '1' }] }
  ];
  ...
  assert.equal(observation.crossFile[0].relation, 'forward');
...
test('batch summary redacts identifiers but preserves descriptive boundary text', () => {
  ...
  const entries = [
    ...
    { name: 'b.csv', rows: [{ timestamp_s: '10', phase: 'steady', current_a: '10', voltage_v: '20', temperature_c: '25', pressure_bar: '1', flow_slpm: '3', leak_ppm: '1' }], rowCount: 1, hash: 'hash-b' }
  ];
  const fileResults = entries.map((entry) => ({ name: entry.name, result: analyzeRows(entry.rows) }));
  ...
  assert.equal(publicSummary.testRunIdPresent, true);
  assert.equal(publicSummary.sessionGroupPresent, true);
  assert.ok(typeof publicSummary.boundary === 'undefined' || typeof publicSummary.boundary === 'string');
});
