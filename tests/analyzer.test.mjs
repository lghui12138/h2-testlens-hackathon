...
test('accepts stack stoichiometry when gas flow headers declare actual L/min and treats it as SLPM', () => {
  const result = analyzeRows(parseCSV([
  ...
  assert.equal(result.quality.usable, true);
  assert.equal(result.rows[0].anode_flow_slpm, 60);
  assert.equal(result.rows[0].cathode_flow_slpm, 120);
  assert.ok(result.dataset.metrics.hydrogenStoich.mean > 0);
  assert.ok(result.dataset.metrics.airStoich.mean > 0);
